...
const MonthModalTodo = ({dayInfo, handleModalClose}) => {
    ...
    console.log(todos)
  return (
    <div className='absolute inset-x-0 w-96 h-96 mx-auto bg-gray-200 text-center select-none'>
      ...
      <div>todo
        ...
        <div className='text-left m-5 w-48 h-48 bg-white mx-auto'>
          {todos.map((todo, idx)=>(
            <li key={idx}>{todo.text}<Button onClick={()=>onDelete(todo)} content="DELETE"></Button></li>
          ))}
        </div>
        <div>
