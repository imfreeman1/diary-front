...
const STICKER=(id,imgURL,positionX,positionY,width,height,selected)=>{
  return {
    id,
    imgURL,
    positionX,
...
export const stickerSlice = createSlice({
  ...
  reducers: {
    setSticker: ({stickersObj }, {
      payload: {
        id, position:{positionX,positionY}, origin, newId, pageDate,
      },
    }) => {
      ...
      const newSticker = { ...selectedSticker,id: newId,positionX,positionY };

      if(!stickersObj[origin][pageDate]) 
    ...
    getStickers: ({ stickersObj }, { payload: { origin, getStickerArray, pageDate } }) => {
      stickersObj[origin][pageDate] = getStickerArray.map(({id,image,position,size}) => 
    ...
    setPosition: ({ stickersObj }, {
      payload: {
        origin, id, position:{positionX,positionY}, pageDate,
      },
    }) => {
    ...
    setResize: (
      ...
      {
        payload: {
          origin, id, size:{width,height}, position:{x,y}, pageDate,
        },
      },
    ) => {
      const foundedStickerArr = stickersObj[origin][pageDate];
    ...
    addTableSticker: ({ stickersObj }, { payload: { imgURL, size:{width,height} } }) => {
