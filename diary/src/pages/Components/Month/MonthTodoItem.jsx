...
const MonthTodoItem = ({todo, dayInfo}) => {
  ...
  return (
    <>
      <li key={v4()} onClick={()=>handleItemModalOpen()} className="block truncate bg-gray-200 p-1 pl-2 my-2 mx-0.5 border-gray-400 border-2 rounded-xl hover:bg-gray-300 hover:border-gray-600 hover:cursor-pointer">{todo.text}</li>
      <div onDoubleClick={onChildDbclick} ref={editModalRef}>
          <MonthEditModal todo={todo} dayInfo={dayInfo} itemModalVisible={itemModalVisible} handleItemModalClose={handleItemModalClose}></MonthEditModal>
      </div>
    </>
  )
}
