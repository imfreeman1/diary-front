...
import { DAY_OF_WEEK, MONTH_LIST } from './Constants/monthConstants';
import { v4 } from "uuid"
    ...
    <div className='flex justify-center pt-5 h-screen w-full bg-gray-100'>
      <div className='bg-zinc-50 m-5 h-fit border'>
        <div className='flex bg-zinc-50 gap-5'>
          ...
          <p className='text-5xl w-fit px-6 m-3 text-gray-700 select-none'>{selectedMonth+1}{MONTH_INDICATING} {yearInMonth}</p>
        </div>
        <div className='flex my-2 border-2'>
          {DAY_OF_WEEK.map((day)=> 
            <div className={`flex border w-36 text-lg font-bold justify-center ${day==="Sun"? "text-[#FF0000]":""}`} key={v4()}>{day[0]}</div>
        )}
        </div>
        <table className='border-collapse border border-gray-500'>
          {monthCalendar.length?monthCalendar.map((week) => {
