...
const MonthEditModal = ({todo, dayInfo, itemModalVisible, handleItemModalClose}) => {
  ...
  return (
    (itemModalVisible && dayInfo.locdate ?
      <div className='z-0 absolute inset-x-auto w-96 h-fit bg-white text-right select-none rounded drop-shadow-2xl'>
          <div className='m-3 flex flex-row-reverse'>
            ...
            <BiEdit onClick={()=>setIsEdited(true)} size="25" className='rounded cursor-pointer hover:bg-gray-200' />
          </div>
        <div className='text-left m-10'>
            {isEdited ? 
            <form onSubmit={handleKeyPress}>
                <span>■ </span><input placeholder='입력하세요' value={editText} ref={focusRef} onChange={handleEditText} className="text-xl border-b-2"/>
            </form>
